...
def compiler_compta_membres(liste_flux, dict_capitaux, membres_inscrits):
    """
    Compile la compta hebdomadaire des membres selon le barème d'activité :
    - Achat / Consommation Interne = 0.5 point / u
    - Réapprovisionnement des stocks = 0.1 point / u
    - Capital & Apport initial = 0 point (neutralisé)
    """
    # Initialisation de la structure comptable brute
    compta = {
        m: {
            ...
            "Score d'Apport Total": 0.0
        } for m in membres_inscrits
    }

    # 1. Ventilation et cumul des volumes de matériaux (u)
    for fl in liste_flux:
        user = str(fl.get("joueur", "")).strip()
        t_mouv = fl.get("type")
        cash = float(fl.get("apport_cash", 0.0))
        
        mats_dict = fl.get("materiaux", {})
        mats_qte = float(sum(mats_dict.values())) if isinstance(mats_dict, dict) else 0.0

        # Gestion des livraisons de réapprovisionnement
        if t_mouv == "REAPPROVISIONNEMENT":
            if user in compta:
                compta[user]["Réappro Matériaux (u)"] += mats_qte
            elif user in ["Réapprovisionnement Global", ""]:
                # Répartition équitable si la ligne est anonyme
                for m in membres_inscrits:
                    compta[m]["Réappro Matériaux (u)"] += (mats_qte / len(membres_inscrits))

        # Gestion des achats marchands
        elif user in compta:
            if t_mouv == "REINVESTISSEMENT_CASH":
                compta[user]["Réinvestissements (€)"] += cash
                
            elif t_mouv in ["ACHAT_INTERNE", "ACHAT_EXTERNE"]:
                compta[user]["Consommation Interne (u)"] += mats_qte
                compta[user]["Bénéfices Générés (€)"] += (mats_qte * 1.0)

    # 2. Application mathématique stricte du barème d'effort d'activité
    for m in compta:
        # L'effort de réapprovisionnement rapporte 0.1 point par unité
        reappro_pts = compta[m]["Réappro Matériaux (u)"] * 0.1
        
        # L'achat de matériel rapporte 0.5 point par unité
        consom_pts = compta[m]["Consommation Interne (u)"] * 0.5
        
        # Le score total de la semaine ignore le capital et additionne les points d'effort
        compta[m]["Score d'Apport Total"] = round(reappro_pts + consom_pts, 1)
