...
def init_db():
    ...
    try:
        ...
        pass
    try:
        cursor.execute("ALTER TABLE chantiers ADD COLUMN roi_par_jour REAL DEFAULT 0")
    except sqlite3.OperationalError:
        pass
    conn.commit()
    conn.close()
...
# --- CONFIGURATION DES ONGLETS (AVEC ESPACE DIRECTION PROTÉGÉ) ---
...
with onglet3:
    ...
    if mot_de_passe == "adminBTP2026":
        ...
        if df_stats.empty:
            ...
            with c_st3:
                st.metric(label="📉 Investissements / Dépenses", value=f"{txt_total_dep} €")
            with c_st4:
