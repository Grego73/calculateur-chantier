# Contenu de : onglets/suivi_interne/tab_diagnostic.py
import streamlit as st
import pandas as pd
import database as db

def afficher_tab_diagnostic(nom_coop_active):
    ...
    st.markdown(f"### 📋 Base de données brute complète pour : `{nom_coop_active}`")
    
    try:
        # Requête réseau vers la sous-collection Firestore basée sur la coopérative connectée
        doc_ref_coop = db.db.collection("cooperatives").document(nom_coop_active)
        ...
        if liste_documents_bruts:
            ...
            if st.button("🔥 SUPPRIMER DÉFINITIVEMENT LES LIGNES SÉLECTIONNÉES", type="primary", use_container_width=True, key="btn_purge_maximale"):
                ...
                if lignes_a_supprimer.empty:
                    ...
                    db.enregistrer_log("NETTOYAGE_EXPLICITE", f"Purge manuelle intégrale de {compteur_suppressions} document(s) pour {nom_coop_active}.")
                    st.success(f"💥 {compteur_suppressions} document(s) NoSQL effacé(s) de votre serveur cloud !")
                    st.cache_data.clear()
                    if hasattr(db, 'charger_flux_coop_cache'):
                        db.charger_flux_coop_cache.clear()
                    st.rerun()
        else:
            st.info(f"💡 Aucun document trouvé dans la table de la coopérative {nom_coop_active}.")
